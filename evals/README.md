...
1. **Реперная точка.** Выбираем коммит `C0`. Память строится скиллом по состоянию `C0`
...
3. **Два изолированных прогона из одного снапшота** (git worktree на `C0`):
   - `memory-off`: агент + задача;
   - `memory-on`: тот же агент + задача + релевантные active-факты (журналируем MemoryRead — какие
     именно факты вошли).
