...
def report(rows: list[dict], baseline: dict[str, int],
    ...
    # --- 5. цена памяти -------------------------------------------------------
    ons = [p["memory-on"] for p in pairs.values()]
    ctx = [r["context_chars"] for r in ons]
    if any(ctx):
        modes = {r.get("memory_mode") for r in ons if r.get("memory_mode")}
        mode = ", ".join(sorted(modes)) if modes else "не записан"
        w("## Цена памяти\n")
        w(f"Режим памяти: **{mode}**. В контекст уезжает "
          f"{statistics.mean(ctx):,.0f} символов (~{statistics.mean(ctx) / 4:,.0f} токенов) "
          f"на сессию.\n")

        # Разные задачи — разный пакет? Считаем по хешу содержимого, а не по длине:
        # два разных набора фактов легко совпадают по числу символов.
        shas = [r.get("context_sha") for r in ons if r.get("context_sha")]
        by_task: dict[str, set] = defaultdict(set)
        for (t, _), p in pairs.items():
            v = p["memory-on"].get("context_sha") or p["memory-on"]["context_chars"]
            by_task[t].add(v)
        distinct = len({next(iter(v)) for v in by_task.values() if len(v) == 1})
        w("| задача | символов в контексте |")
        w("| --- | ---: |")
        for t in sorted(by_task):
            sizes = {p["memory-on"]["context_chars"] for (tt, _), p in pairs.items() if tt == t}
            w(f"| {t} | {', '.join(f'{x:,}' for x in sorted(sizes))} |")
        w("")

        same_everywhere = (len(set(shas)) == 1 if shas else len(set(ctx)) == 1)
        if same_everywhere:
            if "xmemcli" in mode:
                w("> **Режим `xmemcli`, но на все задачи приезжает один и тот же пакет.** "
                  "Значит память ничего не отбирает под задачу, и сравнение с `memory-off` "
                  "меряет объём контекста, а не пользу системного дизайна — ровно как в режиме "
                  "снапшота. Чинится сужением запроса: `[memory] context_args` в конфиге, "
                  "id задачи хук получает в `KATA_TASK_ID`.\n")
            else:
                w("> Всем задачам инжектится **один и тот же** снапшот. Проверяется не "
                  "«релевантная память помогает», а «полный дамп фактов в промпте помогает». "
                  "Пока факты не отбираются под задачу, тезис о системном дизайне не проверен.\n")
        elif distinct > 1:
            w(f"> Память отдаёт **разные пакеты разным задачам** ({distinct} различных). "
              "Это то, чего не было в прогоне со снапшотом: теперь сравнение off/on "
              "проверяет именно отбор фактов, а не объём текста. Смотри, коррелирует ли "
              "размер приехавшего пакета с результатом — если нет, отбор работает, но "
              "содержимое не по делу.\n")
