...
Для `agent.model` обязателен полный датированный Claude model id (`claude-…-YYYYMMDD`):
плавающие алиасы раннер отвергает, чтобы cutoff и повторный запуск означали одну модель.
...
- **Модель и CLI записываются.** В `metrics.json` и CSV остаются полный model id, версия CLI и
  cache-creation/read токены.
