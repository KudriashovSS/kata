...
def install_agent_runtime(wt: Path, cfg, memory_on: bool) -> dict[str, str]:
    ...
    if memory_on:
        ...
        # Место, куда положить сужение запроса к памяти под задачу, не трогая хук.
        # Пока пусто, xmemcli отдаёт один и тот же пакет на все задачи — и тогда
        # сравнение с memory-off меряет объём контекста, а не пользу отбора.
        env["KATA_XMEM_CONTEXT_ARGS"] = cfg["memory"].get("context_args", "")
    return env
...
def main() -> int:
    ...
    try:
        ...
        metrics = {
            ...
            # хеш нужен, чтобы автоматически ловить «памяти прилетает одно и то же
            # на любую задачу» — при таком раскладе сравнение меряет объём контекста
            "context_sha": (hashlib.sha256(ctx.read_bytes()).hexdigest()[:12]
                            if ctx.exists() and ctx_chars else None),
        }
